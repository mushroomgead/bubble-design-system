...
└── apps/
    └── docs/
        ...
        └── app/
            ├── layout.tsx          # html/body shell with bg-surface text-content
            ├── globals.css         # @import preset.css + @source pointing at packages/ui/src
            └── page.tsx            # Button gallery — "use client", variants × sizes × states + override examples
```
...
# → expect:
#   ESM dist/index.js      ~1.45 KB
#   CJS dist/index.cjs     ~1.52 KB
...
### Task 2 (DONE): Scaffold `apps/docs` to preview components

Shipped at `apps/docs/`. Stack: Next.js 16 (Turbopack) + MDX + Tailwind v4. Imports `@plain-ds/ui/preset.css` and uses `@source` to scan the lib's source for Tailwind classes. Gallery page (`app/page.tsx`) renders Button across all variants × sizes × states + className-override examples. Run: `pnpm -C apps/docs dev` → http://localhost:3000. MDX is wired (pageExtensions includes `mdx`) for future per-component docs, but the gallery itself stays TSX.
