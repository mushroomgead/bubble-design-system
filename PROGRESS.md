...
**Last updated:** 2026-06-14 (Created formal ADRs for the active architectural decisions in `docs/decisions/`)
...
### 3.16 New component family — Chat (2026-06-14)

- **Source:** third Anthropic-bundled handoff from `claude.ai/design`, fetched as `https://api.anthropic.com/v1/design/h/5E2rvZVfMsSiHpvTWijgTw` (a "bubble-design-system" project). The relevant transcript was `chats/chat6.md` ("Chat Message Component"), with prototype output `project/src/chat.jsx` (vanilla JSX via `window.React`), `project/styles/chat.css`, and a reference demo `project/preview/components-chat.html` (`@dsCard` "Chat" — title bar, date divider, 5 messages, compose bar). As with §3.12/§3.15, only the visual spec and component shapes transferred — code was re-derived through the existing Base UI/`cn()`/BEM-CSS pattern; this was the only component family in the bundle not already present in the library.
- **`src/components/Chat.tsx` + `.css`** — four exports, all simple `forwardRef`/plain function components (no Base UI primitive needed — chat bubbles/threads are plain markup):
  - **`ChatThread`** — `forwardRef<HTMLDivElement>` scroll container, `pds-chat-thread`.
  - **`ChatDateDivider`** — `forwardRef<HTMLDivElement>`, `role="separator"`, renders `children` as the centered label (`pds-chat-divider__label`). The prototype used a `label` string prop + redundant `aria-label`; per the "content via `children`" rule, this became a `children`-only component since the label is the element's only visible content.
  - **`ChatMessage`** — `forwardRef<HTMLDivElement>` extending `ComponentPropsWithoutRef<"div">`. Props: `side` (`"sent" | "received"`, default `"received"`), `position` (`"solo" | "first" | "middle" | "last"`, default `"solo"`), `avatar?: ReactNode`, `name?`, `time?`, `status?: "sending" | "sent" | "delivered" | "read"`, `reactions?: ChatReaction[]` (`{ emoji, count?, mine? }`). Derives `showMeta` (name/time row, solo/first), `showAvatar` (received + solo/last), `showStatus` (sent + solo/last) from `side`/`position`. Renders meta-row, then avatar-slot + bubble (+ reaction pills), then delivery-status row.
  - **`ChatCompose`** — plain function component (matches `DataTable`/`CommandPalette`'s non-forwardRef, non-div-extending shape). Props `{ value?, onChange?, onSend?, placeholder, avatar?: ReactNode, disabled?, className? }`, controlled/uncontrolled via `value !== undefined`. Auto-growing `<textarea>` via `useRef` + `useEffect` (`scrollHeight` capped at 120px, same approach as the prototype's vanilla-JS auto-grow). Enter sends (Shift+Enter newline); send button gets `pds-chat-send--active` once `text.trim()` is non-empty.
  - Local non-exported icon functions: `ChatSendIcon`, `ChatPaperclipIcon`, `ChatSmileIcon`, `ChatDeliveryIcon({ status })` (three glyphs: dashed circle for "sending", single check for "sent", double check for "delivered"/"read").
- **Avatar API mismatch → `avatar?: ReactNode` slot prop:** the prototype's Chat components took `name`/`src` and derived initials internally (its own bespoke `Avatar`). This library's `Avatar` is the compound `{ Root, Image, Fallback }` from §"MVP batch 1" with no initials-from-name helper. Rather than add name→initials logic to the library, `ChatMessage`/`ChatCompose` accept `avatar?: ReactNode` and the consumer passes a fully-composed `<Avatar><Avatar.Fallback>LT</Avatar.Fallback></Avatar>` — consistent with "composable, not opinionated".
- **Scope discipline — no `onReactionClick`:** reaction pills are `<button>`s (so they're focusable/clickable for future wiring), but no click-handler prop was added since chat6.md's spec only covers rendering (`reactions: [{ emoji, count, mine }]`), not interaction.
- **CSS architecture — no `[data-tone="soft"]` overrides (continuing §3.12/§3.13/§3.15):** `chat.css`'s `[data-tone="soft"]` blocks (border removal, flattened radii, `color-mix`-based brand-tinted glows on the sent bubble and send button) were **not** carried forward. Instead:
  - Received/sent bubbles and reaction pills use `box-shadow: var(--shadow-sm)` with no border — already tone-aware.
  - Bubble corner radii use `var(--radius-xl)` (base) / `var(--radius-sm)` (flat "tail" corner) instead of hardcoded `18px`/`4px` — these scale sensibly across the `[data-radius]` axis (under `pill`, `--radius-sm` becomes `9999px`, so bubbles degrade to full pills with no flat corner — judged an acceptable look).
  - The sent bubble and active send button use plain `var(--shadow-sm)`/`var(--shadow-md)` instead of the prototype's brand-tinted `color-mix` glow.
  - **Compose bar** needs no `[data-tone]` branch either: `.pds-chat-compose` background is `var(--color-bg-secondary)` (equals `--color-bg-page` in soft tone, a light gray distinct from white in vivid/pastel), while `.pds-chat-compose__field` is `var(--color-bg-primary)` with `box-shadow: var(--shadow-md)` — "white pill floating on gray" in soft, "white card on light-gray bar" in vivid/pastel, with zero branching. The field's radius is `var(--ctrl-radius)` (pill in soft, `--radius-md` elsewhere) instead of a hardcoded `22px`. Compose bar border is `border-top: 1px solid var(--color-border-tertiary)`. Icon/send buttons keep `var(--radius-full)` (always circular, decorative — not on the radius axis).
- **Exports:** `ChatThread`, `ChatDateDivider`, `ChatMessage`, `ChatCompose` + types `ChatThreadProps`, `ChatDateDividerProps`, `ChatMessageProps`, `ChatComposeProps`, `ChatMessageSide`, `ChatMessagePosition`, `ChatMessageStatus`, `ChatReaction`, added to `src/index.ts` after the `CommandPalette` block. `Chat.css` sorts alphabetically between `Card.css` and `Checkbox.css` in `build-css.mjs`'s glob — no special-casing needed.
- **Docs gallery:** `apps/docs/app/page.tsx` gained a "Chat" section between "Command Palette" and "Brand mark · gradient blob", containing a `ChatDemo` component rendered inside a `.docs-chat-frame` (white surface, `border-radius: var(--radius-2xl)`, `box-shadow: var(--shadow-lg)`, fixed 520px height, flex column, `overflow: hidden` — mirrors `components-chat.html`'s `.chat-frame`). Contents: a title bar (`Avatar` "LT" fallback + name "Lena Torres" + `StatusPill` "Online" + mono "Bubble · chat" meta, new `.docs-chat-frame__bar*` helper classes in `globals.css`), a `ChatThread` reproducing all 5 messages from the preview HTML (received first/last with `reactions={[{emoji:"👍",count:2,mine:true},{emoji:"🔥"}]}`, sent solo `status="read"`, sent first, sent last `status="sent"`) plus any messages appended via local `useState`, and a `ChatCompose` with an "AK" avatar slot whose `onSend` appends a new `status="sending"` message to the thread.
- **Verification:** `pnpm -C packages/ui typecheck` / `build`, `pnpm -C apps/docs typecheck` / `build`, `pnpm -C packages/ui lint`, `pnpm -C apps/docs lint`, `pnpm format:check` — all pass. `dist/styles.css` grew from 24 to 25 components (~80.6 KB). Confirmed via dev server + `curl` that the rendered page includes all `pds-chat-*` and `docs-chat-frame*` classes and that the compiled CSS preserves `var(--…)` token references (live theme switching intact).

### 3.17 README discoverability — Common UI Patterns table, Tokens-vs-Components callout, Popover/DataTable/CommandPalette/Chat doc backfill, new `llms.txt` (2026-06-14)

- **Source:** `SPEC-pattern-component-discoverability.md` (user-provided spec, 2026-06-14). Observed pattern across consumer apps: an AI agent integrating `@bubble-design-system/ui` imports `styles.css`, uses a handful of "obvious" components, then hand-rolls everything else (message bubbles, status banners, command menus) directly against design tokens — because `README.md` had no "I want to build X → use Y" lookup and was stale relative to `src/index.ts` (said "21 components", didn't document `Popover`/`DataTable`/`CommandPalette` at all).
- **Scope expansion beyond the spec:** the spec frames this as "21 → 24 components" (`Popover`/`DataTable`/`CommandPalette` from v1.1.0, §3.15). But §3.16's **Chat** family shipped the same day and was _also_ undocumented — `dist/styles.css` is at **25** component CSS files, not 24. Chat is also the textbook answer to the spec's own motivating example ("build a chat message bubble"), which the spec's draft pattern table still mapped to `Card`. Folded Chat into the backfill and the pattern table so the README wouldn't go stale again immediately.
- **What shipped, all in `packages/ui/`:**
  - New `## Common UI Patterns` section in `README.md`, placed immediately after "Design principles" (before the Table of contents) — a "you're building X → use Y, not raw tokens" decision table covering all 25 components, including a new row for Chat (`ChatThread`/`ChatMessage`/`ChatCompose`) and a reworded `Card` row (no longer claims "message bubble", now owned by the Chat row).
  - New "Tokens vs. Components" callout at the top of `## Design tokens`, redirecting readers to Common UI Patterns before they hand-roll something with `var(--…)`.
  - Backfilled `### Chat`, `### CommandPalette`, `### DataTable`, `### Popover` sections (sub-component/props tables + usage examples, alphabetically placed) — the per-component reference now covers all 25 components/families.
  - Intro bullet and the "Components" import example updated from "21 components" to "25 components", adding `Popover`, `DataTable`, `CommandPalette`, `Chat`.
  - New `packages/ui/llms.txt` — a short AI-first pattern→component lookup + full component list + setup snippet, for agents that only fetch a small context file.
...
│       │   │   ├── Chat.tsx + .css        # ChatThread/ChatDateDivider/ChatMessage/ChatCompose; grouped bubbles, reactions, delivery status, auto-grow compose
...
#   ✓ dist/styles.css      ~80.6 KB (25 components + tokens + base)
